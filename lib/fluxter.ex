defmodule Fluxter do
  ...
  defmacro __using__(_opts) do
    ...
    quote bind_quoted: [pool_size: pool_size], location: :keep do
      ...
      def write(measurement, tags, fields, timestamp_nano_secs) when is_list(fields) do
        System.unique_integer([:positive])
        |> rem(@pool_size)
        |> worker_name()
        |> Fluxter.Conn.write(measurement, tags, fields, timestamp_nano_secs)
      end

      def write(measurement, tags, value, timestamp_nano_secs)
          when is_float(value) or is_integer(value)
          when is_boolean(value) or is_binary(value) do
        write(measurement, tags, [value: value], timestamp_nano_secs)
      end
